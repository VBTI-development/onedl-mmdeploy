...
cd $mmdeploy_dir
echo "Deploy dir:  $mmdeploy_dir"


work_dir=$mmdeploy_dir/work_dir_rtmdet-ins
mkdir -p $work_dir $mmdeploy_dir/data/tiny_coco

model_cfg=$work_dir/rtmdet-ins_s_8xb32-300e_coco.py
checkpoint=$work_dir/rtmdet-ins_s_8xb32-300e_coco_20221121_212604-fdc5d7ec.pth
...
python3 -m mim download onedl-mmdetection --config rtmdet-ins_s_8xb32-300e_coco --dest $work_dir

if [ $backend == "ort" ]; then
    deploy_cfg=configs/mmdet/instance-seg/instance-seg_onnxruntime_dynamic.py
    cp $deploy_cfg configs/mmdet/instance-seg/instance-seg_onnxruntime_dynamic_rtmdet-ins.py
    deploy_cfg=configs/mmdet/instance-seg/instance-seg_onnxruntime_dynamic_rtmdet-ins.py
    echo "onnx_config = dict(input_shape=None)" >> $deploy_cfg
    echo "codebase_config = dict(post_processing=dict(export_postprocess_mask=True))" >> $deploy_cfg

    model=$work_dir/end2end.onnx
elif [ $backend == "trt" ]; then
    deploy_cfg=configs/mmdet/instance-seg/instance-seg_tensorrt-fp16_dynamic-320x320-1344x1344.py
    cp $deploy_cfg configs/mmdet/instance-seg/instance-seg_tensorrt-fp16_dynamic-320x320-1344x1344_rtmdet-ins.py
    deploy_cfg=configs/mmdet/instance-seg/instance-seg_tensorrt-fp16_dynamic-320x320-1344x1344_rtmdet-ins.py
    echo "onnx_config = dict(input_shape=None)" >> $deploy_cfg
    echo "codebase_config = dict(post_processing=dict(export_postprocess_mask=True))" >> $deploy_cfg
    model=$work_dir/end2end.engine
elif [ $backend == "trt-static" ]; then
    deploy_cfg=configs/mmdet/instance-seg/instance-seg_rtmdet-ins_tensorrt_static-640x640.py
    # sdk_cfg=configs/mmdet/instance-seg/instance-seg_sdk_static.py
    # cp $deploy_cfg configs/mmdet/instance-seg/instance-seg_tensorrt-fp16_dynamic-320x320-1344x1344_rtmdet-ins.py
    # deploy_cfg=configs/mmdet/instance-seg/instance-seg_tensorrt-fp16_dynamic-320x320-1344x1344_rtmdet-ins.py
    # echo "onnx_config = dict(input_shape=None)" >> $deploy_cfg
    # echo "codebase_config = dict(post_processing=dict(export_postprocess_mask=True))" >> $deploy_cfg
