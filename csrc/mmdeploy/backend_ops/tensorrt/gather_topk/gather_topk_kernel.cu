...
__global__ void gather_topk_kernel(const scalar_t* input, const int32_t* indices, scalar_t* output,
...
template void gather_topk_impl<float>(const float* input, const int32_t* indices, const int* dims,
                                      int nbDims, const int* indices_dims, int indice_nbDims,
                                      float* output, cudaStream_t stream);

template void gather_topk_impl<int32_t>(const int32_t* input, const int32_t* indices,
                                        const int* dims, int nbDims, const int* indices_dims,
                                        int indice_nbDims, int32_t* output, cudaStream_t stream);
#else
template <typename scalar_t>
__global__ void gather_topk_kernel(const scalar_t* input, const int64_t* indices, scalar_t* output,
                                   int batch, int num_input, int num_indices, int channel) {
  CUDA_1D_KERNEL_LOOP(index, batch * num_indices * channel) {
    const int b_id = index / (num_indices * channel);
    const int n_id = (index / channel) % num_indices;
...
template void gather_topk_impl<float>(const float* input, const int64_t* indices,
                                      const long int* dims, int nbDims,
                                      const long int* indices_dims, int indice_nbDims,
                                      float* output, cudaStream_t stream);

template void gather_topk_impl<int32_t>(const int32_t* input, const int64_t* indices,
                                        const long int* dims, int nbDims,
                                        const long int* indices_dims, int indice_nbDims,
