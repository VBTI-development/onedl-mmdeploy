...
typedef struct mmdeploy_segmentation_t {
  ...
  const void* mask;  ///< segmentation mask pointer (int32_t* or int64_t*), this field might be null
  int mask_dtype;    ///< 0: unknown, 1: int32, 2: int64
  const float* score;  ///< segmentation score map of the input image in CHW format, in which
